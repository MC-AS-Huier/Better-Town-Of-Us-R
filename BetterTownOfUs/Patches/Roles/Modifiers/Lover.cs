...
namespace BetterTownOfUs.Roles.Modifiers
{
    public class Lover : Modifier
    {
        ...
        public static void Gen(List<PlayerControl> canHaveModifiers)
        {
            ...
            var num = BetterTownOfUs.Random.Next(0, crewmates.Count);
            var firstLover = crewmates[num];
            canHaveModifiers.Remove(firstLover);

            var lovingimp = BetterTownOfUs.Random.Next(0, 100);

            PlayerControl secondLover;
            if (CustomGameOptions.LovingImpPercent > lovingimp)
            {
                var num3 = BetterTownOfUs.Random.Next(0, impostors.Count);
                secondLover = impostors[num3];
            }
            else
            {
                var num3 = BetterTownOfUs.Random.Next(0, crewmates.Count);
                while (num3 == num)
                {
                    num3 = BetterTownOfUs.Random.Next(0, crewmates.Count);
                }
